...
template <class DataType> Frame<DataType>::Frame(std::byte *bytes, ssize_t rows, ssize_t cols) {
    this->m_rows = rows;
    this->m_cols = cols;
...
template <class DataType> Frame<DataType>::Frame(ssize_t rows, ssize_t cols, bool allocate_memory ) {
    this->m_rows = rows;
    this->m_cols = cols;
    if (allocate_memory)
