...
// ============================================================================
// fit_pixel / fit_3d — Levenberg-Marquardt template implementations
// ============================================================================
...
NDArray<double, 1> fit_pixel(const FitModel<Model> &model, NDView<double, 1> x,
    ...
    detail::LevenbergMarquardt<Model> solver;
    std::array<double, npar> err{};
    const auto res = solver.fit(model, x, y, y_err, start, result.data(),
                                want_errors ? err.data() : nullptr);
    if (!res.valid)
        return result; // all zeros, like the previous implementation

    if (want_errors) {
        for (std::size_t k = 0; k < npar; ++k)
            result(static_cast<ssize_t>(npar + k)) = err[k];
        result(static_cast<ssize_t>(2 * npar)) = res.chi2;
    } else {
        result(static_cast<ssize_t>(npar)) = res.chi2;
    }
    return result;
}
...
void fit_3d(const FitModel<Model> &model, NDView<double, 1> x,
    ...
    auto process = [&](ssize_t first_row, ssize_t last_row) {
        detail::LevenbergMarquardt<Model> solver; // buffers reused per task
        ...
        for (ssize_t row = first_row; row < last_row; row++) {
            for (ssize_t col = 0; col < y.shape(1); col++) {
                ...
                const auto start = detail::start_values(model, x, values);
                const auto res =
                    solver.fit(model, x, values, errors, start, par.data(),
                               want_par_errors ? err.data() : nullptr);
                ...
                chi2_out(row, col) = res.valid ? res.chi2 : 0.0;
            }
        }
    };
