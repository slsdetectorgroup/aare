...
namespace aare {
void inline merge_frames(std::vector<std::byte *> &part_buffers, size_t part_size, std::byte *merged_frame,
                         const xy &geometry, size_t rows = 0, size_t cols = 0, size_t bitdepth = 0) {
    ...
    if (geometry.col == 1) {
        ...
        const size_t part_rows = rows / geometry.row;
        const size_t part_cols = cols / geometry.col;
