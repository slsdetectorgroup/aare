...
namespace aare {
Pedestal::Pedestal(int rows, int cols, size_t n_samples)
...
NDArray<double, 2> Pedestal::mean() {
    ...
    for (ssize_t i = 0; i < m_rows * m_cols; i++) {
 ...
 */
inline double Pedestal::mean(const int row, const int col) const {
    if (m_cur_samples[index(row, col)] == 0) {
        return 0.0;
    }
    return m_sum[index(row, col)] / m_cur_samples[index(row, col)];
}
inline double Pedestal::variance(const int row, const int col) const {
    if (m_cur_samples[index(row, col)] == 0) {
        return 0.0;
    }
    return m_sum2[index(row, col)] / m_cur_samples[index(row, col)] - mean(row, col) * mean(row, col);
}
inline double Pedestal::standard_deviation(const int row, const int col) const { return std::sqrt(variance(row, col)); }
inline int Pedestal::index(const int row, const int col) const { return row * m_cols + col; }
inline void Pedestal::clear(const int row, const int col) {
